        ...
        <a-row type="flex" justify="space-between" class="content-tool">
          ...
          <a-col>
            <a-row type="flex" justify="start" class="tool-where">
              ...
              <a-col>
                <a-month-picker @change="onChangeDate"   v-model="where.createValue"/>
              </a-col>
  ...
  methods: {
    async getList(num = null, size = null) {
      ...
      let params = {
        ...
        month:this.where.createValue==null?'':this.where.createValue,
        pageNum: this.pageConfig.num,
        pageSize: this.pageConfig.size
      };
      const data = await this.$api.report.getReportMonth(params);
      if (data.code == 0) {
